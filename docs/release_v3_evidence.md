...
Verified on 2026-07-04 America/Costa_Rica against `main` via PR #229 and the
`drchinca/release/v3.0.1` publication branch.
...
| Field | Evidence |
|---|---|
| Package version | `pyproject.toml` declares `3.0.1`; `uv.lock` matches. |
| Python support | `requires-python = ">=3.14"`; CI installs Python 3.14. |
| Package build | `uv build --out-dir /tmp/cemaf-v3-build-check` produced `cemaf-3.0.1.tar.gz` and `cemaf-3.0.1-py3-none-any.whl`. |
| Wheel smoke | Fresh Python 3.14 venv installed the built wheel, imported `cemaf`, asserted `__version__ == "3.0.1"`, constructed `AgentRegistry`, `Context`, `DAG`, `RuntimeServices`, and `create_executor()`, and ran `cemaf --help`. |
...
| REQ-05 | No skipped tests. | `uv run --frozen pytest -q -rs` reported `4117 passed`; skip-pattern scan over `tests src examples pyproject.toml` returned no matches. |
...
| REQ-09 | Direct external comparison/vendor lesson labels are absent. | `check_release_naming.py` passed across 936 files; raw forbidden-name scan returned no matches. |
...
| REQ-12 | Generated docs imports resolve. | `check_doc_imports.py` scanned 95 Markdown files, 331 unique `from cemaf...` imports, 553 total occurrences, 0 failures. |
...
| REQ-14 | Package metadata and docs agree. | `check_release_package.py` passed for `3.0.1`; CI runs the same checker. |
...
| `uv run --frozen pytest -q -rs` | Passed: 4117 passed, no skips reported. |
...
The repository is verified as final package version `3.0.1`. The remaining
publication steps are operational, not framework behavior:

- tag the verified commit as `v3.0.1`;
