...
# ChainContractError imported from SPEC-00 §2 per shared type registry.
...
class NodeInterceptor(ABC):
    ...
    # the default body raises `ChainContractError("correlation_id_unset")` if
    # violated, matching SPEC-00's mint-before-chain contract. The Executor's
    # pre-chain mint (SPEC-00 §"Context extensions") is the load-bearing
    # precondition; unit tests SHALL construct Context with an explicit
    # correlation_id. Raising `ChainContractError` (not `assert`) ensures the
    # check survives `python -O` and surfaces as
    # REJECT(reason="<id>:exception:ChainContractError") with the specced
    # reason string instead of an AssertionError.
...
6. `AgentResult immutability + executor-side construction. Four sub-rules, each independently testable:
   ...
   - 6e. Executor pre-augmentation of tool_calls.consumed_by_node: BEFORE the POST chain runs, the Executor SHALL construct a new AgentResult whose tool_calls tuple has each ToolCallOutput.consumed_by_node populated by inspecting the producing node's STATIC successor edges in the DAG (per SPEC-00 §2 ToolCallOutput contract). This single derivation is the input to the POST chain — interceptors observe consumed_by_node as already populated. The pre-augmentation is the only executor-side AgentResult mutation permitted before POST; it is permitted because consumed_by_node is structurally determined by the DAG (not by chain decisions) and therefore independent of any interceptor's verdict.`
