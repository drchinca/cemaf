...
16. `MetaDispatcher.dispatch SHALL project parent context into RecoveryRequest before sub-DAG construction (the canonical "subagent handoff is a write" boundary, CE rule RULE CE-3): (a) prior_decisions windowed to last PRIOR_DECISIONS_INJECT_WINDOW entries (SPEC-04 Inv 16) plus all entries with kind ∈ {HALT, REJECT}; (b) surfaced_sources filtered to chunks whose Citation appears in the failing node's AgentResult.cited_evidence_refs OR in inbound_hints citations, capped at sum(token_count) ≤ meta_budget.max_token_total / 4. Projection is deterministic so SPEC-00 Property 6 replay holds across nested recoveries.`
