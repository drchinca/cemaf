...
from typing import Any, AsyncIterator # New import

from cemaf.context.budget import TokenBudget
from cemaf.context.compiler import PriorityContextCompiler, ContextSource, CompiledContext, TokenEstimator
from cemaf.context.advanced_compiler import AdvancedContextCompiler
from cemaf.llm.protocols import LLMClient, Message, CompletionResult, LLMConfig
...
class MockLLMClient(LLMClient):
    ...
    async def complete(self, messages: list[Message], tools: list[Any] | None = None, config_override: LLMConfig | None = None) -> CompletionResult:
    ...
    async def stream(self, messages: list[Message], tools: list[Any] | None = None, config_override: LLMConfig | None = None) -> AsyncIterator[Any]:
...
class TestAdvancedContextCompiler:
    ...
    async def test_summarization_when_budget_exceeded(
        ...
        compiled_context = await compiler.compile(
            ...
            memories,
            budget,
            priorities={"brief": 10, "long_mem": 0}
        )        
    ...
    async def test_prioritization_for_summarization(
        ...
        assert next(s for s in compiled_context.sources if "summarized_very_low_prio_mem" in s.key).content == "concise"

        # low_prio_art should also be summarized
        assert any("summarized_low_prio_art" in s.key for s in compiled_context.sources)
        assert next(s for s in compiled_context.sources if "summarized_low_prio_art" in s.key).content == "brief"
    ...
    async def test_llm_summarization_failure_skips_source(
        ...
        compiler = AdvancedContextCompiler(llm_client=failing_llm_client, token_estimator=mock_token_estimator)
        ...
        assert not any("failing content" in s.content for s in compiled_context.sources) # Ensure failed content is not present
        assert not any("summarized_long_mem" in s.key for s in compiled_context.sources) # Ensure failed content is not present
