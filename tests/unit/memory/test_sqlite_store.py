...
async def test_get_missing_returns_none(db_path: str) -> None:
    ...
    store = SqliteMemoryStore(db_path=db_path)
    result = await store.get(scope=MemoryScope.BRAND, key="nonexistent")
...
async def test_set_overwrites_existing(db_path: str) -> None:
    ...
    retrieved = await store.get(scope=MemoryScope.BRAND, key="dup")
    assert retrieved is not None
...
async def test_delete_existing(db_path: str) -> None:
    ...
    assert await store.delete(scope=MemoryScope.BRAND, key="to_delete") is True
    assert await store.get(scope=MemoryScope.BRAND, key="to_delete") is None


async def test_delete_nonexistent(db_path: str) -> None:
    """Delete returns False for non-existent items."""
    store = SqliteMemoryStore(db_path=db_path)
    assert await store.delete(scope=MemoryScope.BRAND, key="nope") is False


async def test_list_by_scope(db_path: str) -> None:
    """List returns only items matching the requested scope."""
    store = SqliteMemoryStore(db_path=db_path)
    await store.set(item=_make_item(scope=MemoryScope.BRAND, key="b1"))
    await store.set(item=_make_item(scope=MemoryScope.BRAND, key="b2"))
    await store.set(item=_make_item(scope=MemoryScope.PROJECT, key="p1"))

    brand_items = await store.list_by_scope(scope=MemoryScope.BRAND)
...
async def test_cleanup_expired(db_path: str) -> None:
    ...
    assert await store.get(scope=MemoryScope.BRAND, key="expired") is None
    assert await store.get(scope=MemoryScope.BRAND, key="alive") is not None
...
async def test_list_by_scope_excludes_expired(db_path: str) -> None:
    ...
    items = await store.list_by_scope(scope=MemoryScope.BRAND)
    assert len(items) == 1
...
async def test_persistence_across_instances(db_path: str) -> None:
    ...
    retrieved = await store2.get(scope=MemoryScope.BRAND, key="persistent")
...
async def test_scope_path_preserved(db_path: str) -> None:
    ...
    retrieved = await store.get(scope=MemoryScope.BRAND, key="pathed")
    assert retrieved is not None
...
async def test_scope_path_none_preserved(db_path: str) -> None:
    ...
    retrieved = await store.get(scope=MemoryScope.BRAND, key="no_path")
    assert retrieved is not None
