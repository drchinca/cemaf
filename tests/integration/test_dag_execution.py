...
from cemaf.orchestration.executor import DAGExecutor, NodeResult, NodeExecutor
...
class TestDAGExecution:
    ...
    async def test_simple_linear_dag(self, simple_dag: DAG, mock_node_executor: MockNodeExecutor):
        ...
        result = await executor.run(simple_dag, initial_context=Context(data={"initial": "data"})) # Updated initial_context
    ...
    async def test_conditional_edge_on_success(self, mock_node_executor: MockNodeExecutor):
        ...
        dag = dag.add_node(Node.tool(id="start", name="Start", tool_id="t1", output_key="start_out")) # Added output_key
        dag = dag.add_node(Node.tool(id="success_path", name="Success", tool_id="t2", output_key="success_path_out")) # Added output_key
    ...
    async def test_failure_handling(self):
        ...
        mock = MockNodeExecutor(node_results={
            "bad": NodeResult(node_id=NodeID("bad"), success=False, error="Failed!")
        })
    ...
    async def test_context_propagation(self, mock_node_executor: MockNodeExecutor):
        ...
        executor = DAGExecutor(node_executor=mock_node_executor)
        result = await executor.run(dag, initial_context=Context(data={"initial_data": "hello"})) # Updated initial_context
    ...
    async def test_router_selects_route(self, mock_node_executor: MockNodeExecutor):
        ...
        executor = DAGExecutor(node_executor=mock_node_executor)
        result = await executor.run(dag, initial_context=Context(data={"route": "success"})) # Updated initial_context
    ...
    async def test_conditional_node_branches(self, mock_node_executor: MockNodeExecutor):
        ...
        executor = DAGExecutor(node_executor=mock_node_executor)
        result = await executor.run(dag, initial_context=Context(data={"ok": True})) # Updated initial_context
    ...
    async def test_parallel_node_executes_children_once(self, mock_node_executor: MockNodeExecutor):
        ...
        dag = dag.add_node(Node.parallel(
            id="parallel",
            name="Parallel",
            parallel_nodes=["a", "b"],
