...
from cemaf.core.enums import AgentStatus, RunStatus
...
@dataclass(frozen=True)
class DeepAgentResult(Generic[ResultT]):
    """
...
class DeepAgentOrchestrator:
    ...
    def _check_timeout(self) -> None:
        ...
        if elapsed > self._config.timeout_seconds:
            raise asyncio.TimeoutError(f"Timeout exceeded: {elapsed:.1f}s > {self._config.timeout_seconds}s")
    ...
    async def spawn_child(
        ...
        if current_children >= self._config.max_children_per_agent:
            error_msg = f"Max children per agent ({self._config.max_children_per_agent}) exceeded for {parent_id}"
            if self._config.propagate_errors:
