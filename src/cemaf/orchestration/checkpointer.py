...
class CheckpointingDAGExecutor:
    ...
    async def run(
        ...
        await self._checkpointer.save(DAGCheckpoint(
            run_id=run_id,
            dag_name=dag.name,
    ...
    async def _execute_nodes(
        ...
        try:
            for node_id in nodes_to_execute:
                ...
                result, context = await self._executor._execute_with_retry(node, context) # Context is updated here
                ...
                if nodes_since_checkpoint >= self._interval:
                    await self._save_checkpoint(
                        run_id, dag.name, RunStatus.RUNNING,
                        completed_nodes, nodes_to_execute, context,
                    )
                    nodes_since_checkpoint = 0
                ...
                if not result.success:
                    if self._checkpoint_on_failure:
                        await self._save_checkpoint(
                            run_id, dag.name, RunStatus.FAILED,
                            completed_nodes, nodes_to_execute, context,
                            error=result.error, failed_node=node_id,
                        )
            ...
            # Success
            await self._save_checkpoint(
                run_id, dag.name, RunStatus.COMPLETED,
